"""Check saved #43 evidence bytes and result consistency; does not run UPPAAL."""
import hashlib
import json
...
def main():
    ...
    assert sum(len(r['per_query']) for r in results) == publication['query_results'] == 34
    assert b'Ran 175 tests' in files['software.stderr.txt']
    assert files['software.stderr.txt'].rstrip().endswith(b'OK')
    print(f"Evidence integrity OK: {len(files)} files, 34 recorded query results, 175 recorded software tests. No new model checking.")
