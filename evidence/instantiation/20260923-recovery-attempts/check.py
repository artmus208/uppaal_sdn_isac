"""Run immutable #45 diagnostics from a clean source commit; no P3/Gate claims."""
import argparse
import hashlib
...
def main():
    ...
    cases=[('integrated-parser',c.model_xml.encode(),[('E<> true',True)],
            'Full model engine load only; no full-model recovery-attempt result')]
    ...
    execute('diff',['git','diff','--check','6890960b3e0042b633887957052aabf45aee51d9',commit])
    ...
    record={'run_id':args.run_id,'source_commit':commit,'source_tree_clean_at_start':True,
            'base_commit':'6890960b3e0042b633887957052aabf45aee51d9','produced_by':'vadimnbkg',
            ...
            'claim_limits':'Restricted recovery-peer diagnostics and synthetic recorder misuse controls only. Full model uses E<> true only; no C01/Gate 1/P3 acceptance.'}
