...
def compile_invocation(executable, output):
    paths = [str(output/'generated/model.xml'), str(output/'generated/queries.q')]
    overrides = {'UPPAAL_COMPILE_ONLY': '1'}
    if os.name != 'nt' and executable.lower().endswith('.exe'):
        # Forward compile-only explicitly across WSL and translate file paths.
        paths = [subprocess.check_output(['wslpath', '-w', path], text=True).strip()
                 for path in paths]
        forwarded = [p for p in os.environ.get('WSLENV', '').split(':')
                     if p and p.split('/')[0] != 'UPPAAL_COMPILE_ONLY']
        overrides['WSLENV'] = ':'.join(forwarded + ['UPPAAL_COMPILE_ONLY/w'])
    return [executable, *paths], overrides
...
def main():
    ...
    if args.verifyta:
        ...
        compile_command, compile_env = compile_invocation(executable, output)
        commands.extend([
            ('verifyta-version',[executable,'--version'],{}),
            ('verifyta-help',[executable,'--help'],{}),
            ('compile-only',compile_command,compile_env),
        ])
