...
def _retry_windows_file_access(operation):
    # Windows readers/replacers may temporarily deny sharing. Bound retries
    # to 0.3 seconds; unrelated and persistent errors still propagate.
...
def read(path):
    return json.loads(_retry_windows_file_access(lambda: Path(path).read_text(encoding='utf-8')))
...
def save(path, value):
    ...
    try:
        with tmp.open('w', encoding='utf-8', newline='\n') as f:
            ...
            f.flush()
            os.fsync(f.fileno())
        _retry_windows_file_access(lambda: os.replace(tmp, path))
    finally:
...
def publish(directory, phase, **kwargs):
    ...
    try:
        ...
        # This snapshot is advisory. Raw telemetry and mandatory result writes
        # remain on the strict path; a busy status reader must not kill search.
        print(f'Warning: status.json update failed; snapshot may be stale: {exc}',
