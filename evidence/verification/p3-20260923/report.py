"""Build the human report from completed, archived execution records."""
import json
from pathlib import Path
HERE=Path(__file__).resolve().parent
ROOT=HERE.parents[2]
a=json.loads((HERE/'p3-20260923-002/results.json').read_text())
b=json.loads((HERE/'p3-20260923-003/results.json').read_text())
index={x['property_id']:x for x in b}
def status(x):
 if x is None:return 'не повторялся'
 return x['verdict'] if x['status']=='success' else x['status']
rows=['| Запрос | BFS, shortest trace | DFS, some trace |','|---|---|---|']
