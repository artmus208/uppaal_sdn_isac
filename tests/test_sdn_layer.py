...
class IntegratedCandidateTests(unittest.TestCase):
    ...
    def test_integrated_operational_context_drift_preserves_model_bytes(self):
        from unittest.mock import patch
        import hashlib
        from uppaal_mcp.integrated.generator import generate
        original = Path.read_bytes
        raw = original(ROOT / 'AGENTS.md') + b'\nChanged operational instructions.\n'
        def changed(path):
            return raw if path == ROOT / 'AGENTS.md' else original(path)
        with patch.object(Path, 'read_bytes', changed):
            candidate = generate(ROOT)
        self.assertEqual(candidate.model_xml, self.candidate.model_xml)
        self.assertEqual(candidate.queries, self.candidate.queries)
        self.assertEqual(candidate.metadata['source_hashes'], self.candidate.metadata['source_hashes'])
        self.assertNotIn('AGENTS.md', candidate.metadata['source_hashes'])
        context = candidate.metadata['context_document_hashes']['AGENTS.md']
        self.assertEqual(context['actual_sha256'], hashlib.sha256(raw).hexdigest())
...
class IntegratedRecorderTests(unittest.TestCase):
    def test_windows_compile_only_forwards_flag_and_translates_paths(self):
        ...
        convert = Mock(side_effect=['C:\\run\\model.xml\n', 'C:\\run\\queries.q\n'])
        ...
        with patch.object(recorder, 'os', SimpleNamespace(name='nt')):
            command, env = recorder.compile_invocation('verifyta.exe', Path('run'))
        self.assertEqual(env, {'UPPAAL_COMPILE_ONLY': '1'})
        self.assertEqual(command[1:], [str(Path('run/generated/model.xml')), str(Path('run/generated/queries.q'))])
