...
def generate_uppaal_model(
    ...
    *,
    ...
    mode_name, include_observers, debug_counters, include_negative_scenarios, include_environment = _normalize_generation_mode(
        mode,
    ...
    builder.system(_system_declaration(include_observers=include_observers, include_environment=include_environment))
    ...
    *,
    ...
    if mode is None:
        if include_negative_scenarios:
            return "with_negative_scenarios", include_observers, debug_counters, True, True
        if include_observers and debug_counters:
            return "with_observers", True, True, False, True
        if debug_counters:
            return "with_debug_counters", include_observers, True, False, True
        if not include_observers:
            return "minimal", False, False, False, True
        return "custom", include_observers, debug_counters, include_negative_scenarios, True
    normalized = mode.strip().lower()
    if normalized == "minimal":
        return "minimal", False, False, False, True
    if normalized == "with_observers":
        return "with_observers", True, debug_counters, include_negative_scenarios, True
    if normalized == "with_debug_counters":
        return "with_debug_counters", include_observers, True, include_negative_scenarios, True
    if normalized == "with_negative_scenarios":
        return "with_negative_scenarios", include_observers, debug_counters, True, True
    if normalized in {"open_system", "open-system", "open"}:
        return "open_system", include_observers, debug_counters, include_negative_scenarios, False
    raise ValueError("Unsupported PHY generation mode. Use minimal, with_observers, with_debug_counters, with_negative_scenarios, or open_system.")
...
def _add_a_ch(builder: UppaalXmlBuilder) -> None:
    ...
    for source, _ in locations:
        if source != "MeasurePending":
            builder.add_transition(template, source, "MeasurePending", sync="measure_tick?", assignment="c_meas = 0")
        builder.add_transition(template, source, source, sync="power_cmd?")
    transitions = [
        ("env_scenario == SCENARIO_NORMAL && c_meas <= D_meas", "ChannelNominal", "ChannelClass = highest_priority_CH(), channel_degraded_flag = false, ch_enabled_count = 1, c_meas = 0"),
        ("env_scenario == SCENARIO_COMM_DEGRADED && c_meas <= D_meas", "InterferenceLimited", "ChannelClass = highest_priority_CH(), channel_degraded_flag = true, ch_enabled_count = 1, c_meas = 0"),
        ("env_scenario == SCENARIO_SENSING_DEGRADED && c_meas <= D_meas", "MobilityLimited", "ChannelClass = highest_priority_CH(), channel_degraded_flag = true, ch_enabled_count = 1, c_meas = 0"),
        ("env_scenario == SCENARIO_JOINT_DEGRADED && c_meas <= D_meas", "Outage", "ChannelClass = highest_priority_CH(), channel_degraded_flag = true, ch_enabled_count = 1, c_meas = 0"),
    ]
...
def _add_a_sig(builder: UppaalXmlBuilder) -> None:
    ...
    for source, _ in locations:
        builder.add_transition(template, source, "SignalReconfiguring", sync="waveform_config?", assignment="c_sig = 0")
        builder.add_transition(template, source, "SignalReconfiguring", sync="pilot_config?", assignment="c_sig = 0")
        builder.add_transition(template, source, "SignalReconfiguring", sync="prs_config?", assignment="c_sig = 0")
        builder.add_transition(template, source, "SignalReconfiguring", sync="payload_sensing_config?", assignment="c_sig = 0")
        builder.add_transition(template, source, "SignalReconfiguring", sync="sensing_mode_cmd?", assignment="c_sig = 0")
    builder.add_transition(template, "SignalReconfiguring", "SignalNominal", guard="c_sig <= D_sig", sync="signal_report!", assignment="SignalClass = SIGNALCLASS_RECONFIGURING, c_sig = 0")
    builder.add_transition(template, "SignalNominal", "PilotBasedSensing", guard="PilotDensityClass == PILOTDENSITYCLASS_OK && BLERClass == BLERCLASS_OK && DRTClass == DRTCLASS_OK", sync="signal_report!", assignment="SignalClass = SIGNALCLASS_PILOT_BASED, c_sig = 0")
    builder.add_transition(template, "PilotBasedSensing", "PayloadAssistedSensing", guard="PilotDensityClass == PILOTDENSITYCLASS_LOW && PayloadSenseClass != PAYLOADSENSECLASS_FAILED", sync="signal_report!", assignment="SignalClass = SIGNALCLASS_PAYLOAD_ASSISTED, c_sig = 0")
    builder.add_transition(template, "PayloadAssistedSensing", "SignalLimited", guard="BLERClass != BLERCLASS_OK || DRTClass == DRTCLASS_BAD || PayloadSenseClass == PAYLOADSENSECLASS_FAILED", sync="signal_report!", assignment="SignalClass = SIGNALCLASS_LIMITED, signal_degraded_flag = true, c_sig = 0")
...
def _add_a_sq(builder: UppaalXmlBuilder) -> None:
    ...
    for sync in ("channel_report?", "signal_report?", "beam_report?"):
        builder.add_transition(template, "Idle", "SensingEvaluating", sync=sync, assignment="input_changed = true, c_sense = 0")
    transitions = [
        ("env_scenario == SCENARIO_NORMAL && c_sense <= D_sense", "SensingQoSOk", "SensingState = highest_priority_SQ(), sensing_degraded_flag = false, sensing_qos_ok = true, sq_enabled_count = 1, c_sense = 0"),
        ("env_scenario == SCENARIO_COMM_DEGRADED && c_sense <= D_sense", "SensingQoSOk", "SensingState = highest_priority_SQ(), sensing_degraded_flag = false, sensing_qos_ok = true, sq_enabled_count = 1, c_sense = 0"),
        ("env_scenario == SCENARIO_SENSING_DEGRADED && c_sense <= D_sense", "FreshnessLimited", "SensingState = highest_priority_SQ(), sensing_degraded_flag = true, sensing_qos_ok = false, sq_enabled_count = 1, c_sense = 0"),
        ("env_scenario == SCENARIO_JOINT_DEGRADED && c_sense <= D_sense", "SensingFailure", "SensingState = highest_priority_SQ(), sensing_failure_flag = true, sensing_qos_ok = false, sq_enabled_count = 1, c_sense = 0"),
    ]
    for guard, target, assignment in transitions:
...
def _add_a_ph(builder: UppaalXmlBuilder) -> None:
    ...
    for source, _ in locations:
        for sync in ("channel_report?", "signal_report?", "beam_report?", "sensing_report?"):
            builder.add_transition(template, source, "PHYKpiReporting", sync=sync, assignment="child_update = true, c_report = 0")
    ...
    transitions = [
        ("env_scenario == SCENARIO_NORMAL && c_report <= D_report", "PHYNormal", "PHYState = PHYSTATE_PHYNORMAL, comm_ok = true, sensing_qos_ok = true, degradation_flag = false, c_report = 0"),
        ("env_scenario == SCENARIO_COMM_DEGRADED && c_report <= D_report", "PHYCommunicationDegraded", "PHYState = PHYSTATE_PHYCOMMUNICATIONDEGRADED, comm_ok = false, sensing_qos_ok = true, degradation_flag = true, c_report = 0"),
        ("env_scenario == SCENARIO_SENSING_DEGRADED && c_report <= D_report", "PHYSensingDegraded", "PHYState = PHYSTATE_PHYSENSINGDEGRADED, comm_ok = true, sensing_qos_ok = false, degradation_flag = true, c_report = 0"),
        ("env_scenario == SCENARIO_JOINT_DEGRADED && c_report <= D_report", "PHYJointDegraded", "PHYState = PHYSTATE_PHYJOINTDEGRADED, comm_ok = false, sensing_qos_ok = false, degradation_flag = true, c_report = 0"),
    ]
    for guard, target, assignment in transitions:
...
def _system_declaration(*, include_observers: bool, include_environment: bool) -> str:
    lines = [
        "A_CH = Template_A_CH();",
