...
def generate_report_bundle(*, contract_json: dict, model_xml: str | None = None, queries: str | None = None, result_json: dict | None = None, trace_text: str | None = None, profile: dict | None = None) -> dict:
    ...
    reports = {
        ...
        "interface_report.md": _interface_report(contract),
...
def _traceability(contract: SdnContractModel) -> str:
    lines = ["# SDN Traceability Matrix", "", "| Claim | IR | UPPAAL artifact |", "|---|---|---|"]
    lines.append("| `A_SDN = A_MON || A_RISK || A_POLICY || A_RULE || A_REC || A_SDN_AGG` | composition | `system A_MON, A_RISK, A_POLICY, A_RULE, A_REC, A_SDN_AGG` |")
    lines.append("| `A_SYS_SDN = A_SDN || A_ENV_SDN` | closed system | `A_ENV_SDN` |")
    for item in contract.automata:
        lines.append(f"| SDN automaton `{item.name}` | locations/guarantees | template `{item.name}` |")
    for item in contract.observers:
        lines.append(f"| bounded response `{item.name}` | observer | `{item.violation_query}` |")
    return "\n".join(lines) + "\n"


def _model_summary(contract: SdnContractModel, *, model_xml: str | None, queries: str | None) -> str:
    lines = ["# SDN Model Summary", "", f"- components: {', '.join(contract.sdn_components)}", f"- env: {', '.join(contract.env_components)}", f"- classes: {len(contract.classes)}", f"- policies: {len(contract.policies)}", f"- properties: {len(contract.properties)}"]
    if model_xml:
...
def _policy_report(contract: SdnContractModel) -> str:
    lines = ["# SDN Policy Report", "", "| Policy | Guard | Mode | Outcome |", "|---|---|---|---|"]
...
def _interface_report(contract: SdnContractModel) -> str:
    lines = ["# SDN Interface Report", "", "| Procedure | Trigger | Deadline | Outcomes |", "|---|---|---|---|"]
    for item in contract.interfaces:
        lines.append(f"| `{item.name}` | `{item.trigger}` | `{item.deadline}` | {', '.join(item.outcomes)} |")
    return "\n".join(lines) + "\n"


def _properties_csv(contract: SdnContractModel, *, result_json: dict | None) -> str:
    out = io.StringIO()
...
def _trace_explanation(trace_text: str, *, result_json: dict | None) -> str:
    ...
    if "obsstale" in lower or "optimistic_reconfig" in lower:
        causes.append("stale/missing telemetry allowed optimistic reconfiguration")
    if "obsrulemiss" in lower or "ruletimeout" in lower or "rule_miss" in lower:
        causes.append("rule miss did not reach install/drop/timeout outcome")
    if "obsadmission" in lower or "service_request" in lower:
        causes.append("service admission did not reach accept/degraded/reject outcome")
    if "obsrecovery" in lower or "recoveryfailed" in lower:
        causes.append("recovery did not reach stable/rollback/failure outcome")
    if "obscommandack" in lower or "commandtimeout" in lower or "ack" in lower:
        causes.append("lower-plane command acknowledgement timeout path")
    if not causes:
        causes.append("unknown SDN trace cause")
    return "# SDN Trace Explanation\n\n" + "\n".join(f"- {item}" for item in causes) + "\n"
